...
TCOD <- read.csv(
  "2.datasets_panel/ES5/ES5_peaks_transcribed_genes_cod_distance_breakpoint_r",
  header = F, sep = "\t")

TH2H <- read.csv(
  "2.datasets_panel/ES5/ES5_peaks_transcribed_genes_h2h_distance_breakpoint_r",
  header = F, sep = "\t")


TTRAN <- read.csv(
  "2.datasets_panel/ES5/ES5_peaks_transcribed_genes_tran_distance_breakpoint_r",
  header = F, sep = "\t")
...
#1.  nt vs tcod
set.seed(123)
b_nt_tcod <- boot2samples(mean, nt, tcod, totSim)

#2. nt vs th2h
set.seed(123)
b_nt_th2h <- boot2samples(mean, nt, th2h, totSim)

#3. nt vs ttran
set.seed(123)
b_nt_ttran <- boot2samples(mean, nt, ttran, totSim)

#4. tcod vs th2h
set.seed(123)
b_tcod_th2h <- boot2samples(mean, tcod, th2h, totSim)

#5. tcod vs ttran
set.seed(123)
b_tcod_ttran <- boot2samples(mean, tcod, ttran, totSim)

#6. th2h vs ttran
set.seed(123)
b_th2h_ttran <- boot2samples(mean, th2h, ttran, totSim)
...
#1. nt vs tcod
set.seed(123)
p_nt_tcod <- perm2samples(mean, nt, tcod, totSim)


#2. nt vs th2h
set.seed(123)
p_nt_th2h <- perm2samples(mean, nt, th2h, totSim)

#3. nt vs ttran
set.seed(123)
p_nt_ttran <- perm2samples(mean, nt, ttran, totSim)

#4. tcod vs th2h
set.seed(123)
p_tcod_th2h <- perm2samples(mean, tcod, th2h, totSim)

#5. tcod vs ttran
set.seed(123)
p_tcod_ttran <- perm2samples(mean, tcod, ttran, totSim)

#6. th2h vs ttran
set.seed(123)
p_th2h_ttran <- perm2samples(mean, th2h, ttran, totSim)
